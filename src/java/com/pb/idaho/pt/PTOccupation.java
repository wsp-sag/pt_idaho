...
public enum PTOccupation implements PTOccupationReferencer {
    ...
    private PTOccupation getOccupationPI(String occupation) {
        ...
        if (occupation.startsWith("1_ManPro") || occupation.startsWith("MANAGER")) {
            return MANAGER;
        }
        if (occupation.startsWith("1a_Health") || occupation.startsWith("HEALTH")) {
            return HEALTH;
        }
        if (occupation.startsWith("2_PstSec") || occupation.startsWith("POST_SECONDARY_ED")) {
            return POST_SECONDARY_ED;
        }
        if (occupation.startsWith("3_OthTchr") || occupation.startsWith("OTHER_ED")) {
            return OTHER_ED;
        }
        if (occupation.startsWith("4_OthP&T") || occupation.startsWith("PROFESSIONAL")) {
            return PROFESSIONAL;
        }
        if (occupation.startsWith("5_RetSls") || occupation.startsWith("RETAIL")) {
            return RETAIL;
        }
        if (occupation.startsWith("6_OthR&C") || occupation.startsWith("OTHER")) {
            return OTHER;
        }
        if (occupation.startsWith("7_NonOfc") || occupation.startsWith("NON_OFFICE")) {
            return NON_OFFICE;
        }
