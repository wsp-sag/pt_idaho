...
import com.pb.idaho.pt.LaborFlows;
...
/**
 * ITDWorkplaceLocationTask is a class that ...
 *
...
public class ITDWorkplaceLocationTask extends MessageProcessingTask {
    ...
    public void readData(){
        synchronized (lock) {
            if (!dataRead) {
                ...
                // reading the work activity mode choice logsums in memory  [AK]
                logsumsInMemory = new Matrix[TourModeChoiceLogsumManager.TOTALSEGMENTS];
                for (int segment = 0; segment < TourModeChoiceLogsumManager.TOTALSEGMENTS; segment++) {
                	logsumsInMemory[segment] = mcLogsums.readLogsumMatrix(ActivityPurpose.WORK, segment);
                }
    ...
    public void runWorkplaceLocationModel(Message msg) {
        ...
        //TourModeChoiceLogsumManager mcLogsums = new TourModeChoiceLogsumManager(globalRb, ptRb);				//[AK]
        ...
        while (index < persons.length) {
            ...
            if(segment != previousSegment){
                wlLogger.info(getName() + ", Finding workplaces for people in segment " + segment);
                	logsum = logsumsInMemory[segment];
                	propensity = laborFlows.calculatePropensityMatrix(logsum, skims.pkDist);
            ...
            if (nPersons > 0) { // there were persons that matched the seg/occ
                ...
                flows = laborFlows.calculateAlphaLaborFlowsMatrix(propensity, logsum, skims.pkDist,
                        segment, occupation);
