...
public class LaborFlows {
    ...
    public void readAlphaValues(TableDataSet production,
        ...
        int tazColumn = production.getColumnPosition("TAZ");
        ...
        try {
			PTOccupationEnumClass = Class.forName(className);
		} catch (ClassNotFoundException e1) {
			e1.printStackTrace();
		}
            
        alphaProduction = new ColumnVector[PTOccupationEnumClass.getEnumConstants().length];
        alphaConsumption = new RowVector[PTOccupationEnumClass.getEnumConstants().length];
        ...
        for (int i = 1; i < labels.length; ++i) {
            ...
            logger.debug("Adding labor production / consumption totals "
                    + occupation.name());
