...
//				System.out.println("Household "+h.sampleNumber+" Person "+p.personNumber);
				...
				if(p.getDayActivities().size()>0){
//					System.out.println("DayActivities>0");
					p.setDayTrips((ArrayList)codeTrips(p.getDayActivities()));
				}
			}
		}
	}
	...
	//Method to code a days' worth of activities into trips, returns a List of trips
	//All four surveys are trip-based, so each record represents a different location,
	//except home, where there can be multiple sequential home activities.
	//At each location (record), up to 5 different activities may be reported.  Method sends
	//these five activities to the prioritizeActivities() method, which returns the
	//activity with the highest priority, with trip information.
